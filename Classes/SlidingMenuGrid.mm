...
- (void) moveToCurrentPage
{
	...
	// Perform the action
	id action = [CCMoveTo actionWithDuration:(fAnimSpeed*0.3f) position:[self GetPositionOfCurrentPage]];
...
- (CGPoint) GetPositionOfCurrentPageWithOffset:(float)offset
{
	...
	: CGPointMake((menuOrigin.x-(iCurrentPage*pageSize)+offset),menuOrigin.y);
}
